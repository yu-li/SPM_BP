...
int CreateSLICSegments(const cv::Mat_<cv::Vec3b>& imgIn, cv::Mat_<int>& segLabels, int numSp, int spSize, int createType, double compactness /*= 20.0*/)
{
    ...
    //---------------------------------------------------------
    if (m_spcount < 20 || m_spcount > sz / 4)
        m_spcount = sz / 200; //i.e the default size of the superpixel is 200 pixels
    if (m_compactness < 1.0 || m_compactness > 80.0)
        m_compactness = 20.0;
    //---------------------------------------------------------
    ...
    if (createType == 1)
        ...
        slic.DoSuperpixelSegmentation_ForGivenSuperpixelSize(img, width, height, labels, numlabels, spSize, compactness);
...
void GetSubImageRangeFromSegments(const cv::Mat_<int>& segLabels, int numOfLabels, int kerLen, cv::Mat_<cv::Vec4i>& subRange, cv::Mat_<cv::Vec4i>& spRange)
{
    ...
    for (iy = 0; iy < height; ++iy) {
        for (ix = 0; ix < width; ++ix) {
            ...
            if (ix < labelExtreme[tmp][0][0])
                labelExtreme[tmp][0][0] = ix;
            if (ix > labelExtreme[tmp][0][2])
                labelExtreme[tmp][0][2] = ix;
            if (iy < labelExtreme[tmp][0][1])
                labelExtreme[tmp][0][1] = iy;
            if (iy > labelExtreme[tmp][0][3])
                labelExtreme[tmp][0][3] = iy;
        }
    }
