        ...
        for (lp = 0; lp < width; ++lp) {
            ...
            while (diff <= maxL) {
                ...
                if (abs(sumColor[0] - img[iy][rp][0] * diff) > tau * diff
                    ...
                    || abs(sumColor[2] - img[iy][rp][2] * diff) > tau * diff)
                    break;
                sumColor += img[iy][rp];
                ++diff;
                ++rp;
        ...
        for (rp = width - 1; rp >= 0; --rp) {
            ...
            while (diff <= maxL) {
                ...
                if (abs(sumColor[0] - img[iy][lp][0] * diff) > tau * diff
                    ...
                    || abs(sumColor[2] - img[iy][lp][2] * diff) > tau * diff)
                    break;
                sumColor += img[iy][lp];
                ++diff;
                --lp;
        ...
        for (up = 0; up < height; ++up) {
            ...
            while (diff <= maxL) {
                ...
                if (abs(sumColor[0] - img[dp][ix][0] * diff) > tau * diff
                    ...
                    || abs(sumColor[2] - img[dp][ix][2] * diff) > tau * diff)
                    break;
                sumColor += img[dp][ix];
                ++diff;
                ++dp;
        ...
        for (dp = height - 1; dp >= 0; --dp) {
            ...
            while (diff <= maxL) {
                ...
                if (abs(sumColor[0] - img[up][ix][0] * diff) > tau * diff
                    ...
                    || abs(sumColor[2] - img[up][ix][2] * diff) > tau * diff)
                    break;
                sumColor += img[up][ix];
                ++diff;
                --up;
